...
Browser support is defined in the `spa_proof_of_concept/browserslist` file that is used for autoprefixing CSS.
...
### Setting up spa-proof-of-concept
...
|run psql                              |`make psql`                            |`docker-compose exec postgres psql --user spa_proof_of_concept --dbname spa_proof_of_concept` |

## Running commands on the server

    docker-compose -f docker-compose.production.yml run --rm --name spa_proof_of_concept_tmp django python manage.py <command>
...
- python: [`spa_proof_of_concept/cover`](./spa_proof_of_concept/cover)
...
make black-check cmd="spa_proof_of_concept/accounts/admin.py" # File path should be relative to project root
